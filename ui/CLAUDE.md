...
| Dashboard.vue | Tableau serveurs + recherche + compteurs + modal ajout serveur (#71) + clé collecteur (#74). Provisionnement atomique via SSH (#299, #301) : SSH user/password obligatoires, serveur créé uniquement si SSH réussit. Validation hostname RFC 1123 (#303). Sur erreur 422, la modal affiche le code d'erreur traduit + bloc `<details>` collapsible avec le `data.details` brut renvoyé par le backend (clamp 500 chars), via `add_server.show_details_summary` (#433). Layout 2 colonnes. |
| ServerDetail.vue | Détail serveur + clés + actions + bandeau rouge si désactivé (#91). Bouton **Edit** (sysadmin) : ouvre `EditServerModal` pour modifier IP, env, OS, port SSH (#339) ; `max_sessions` configurable via `EditServerModal` (seuil alertes sessions, #360). Bandeau orange si `last_scan_ok === false` (#324). Bouton **Re-provisionner** (violet, sysadmin + serveur actif) : modal SSH credentials, spinner, traduction error_code (#302). Sur 422, idem Dashboard — bloc `<details>` collapsible avec `data.details` (#433). `bulkRevokeHasRoot` vérifie les clés composées (`\|root`) — évite faux positif quand un non-root partage le même fingerprint que root. **Bulk revoke fait du targeted per-row** : loop sur les composites, POST `/api/keys/revoke/<fp>` avec `{hostname, unix_user, reason}` au lieu d'un global `/api/keys/bulk-revoke` (qui serait refusé dès que root partage le fingerprint, #429). |
...
| Audit.vue | Historique filtrable + recherche full-text + facets dynamiques pour filtres server/action (#435). `initialLoading` (1ʳᵉ charge plein écran) séparé de `loading` (refetches, passé en prop à AuditTable) — sinon `v-else` démontait la table à chaque frappe et vidait l'input. |
...
| AuditTable.vue | Tableau audit + recherche full-text (debounce 250ms) + filtres serveur (select)/action (select)/date + facets dynamiques (filter-minus-self) + pagination (#250, #435). Selects auto-fetch sur `@change` (pas besoin du bouton Filter), bouton Filter conservé pour la date. Prop `loading` → mini-spinner inline à côté du label Search. |
