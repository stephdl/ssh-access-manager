...
# ---------------------------------------------------------------------------
generate_crontab() {
    # SCAN_INTERVAL_HOURS was documented here and by expire.py but never read:
    # the schedule was hardcoded to every five minutes whatever the operator
    # asked for. Anything outside 1-24 falls back to the five-minute default.
    _schedule="*/5 * * * *"
    case "${SCAN_INTERVAL_HOURS:-}" in
        ''|*[!0-9]*) ;;
        *) [ "$SCAN_INTERVAL_HOURS" -ge 1 ] && [ "$SCAN_INTERVAL_HOURS" -le 24 ] \
               && _schedule="0 */${SCAN_INTERVAL_HOURS} * * *" ;;
    esac

    cat > /etc/crontabs/root << EOF
# ssh-access-manager — generated by bootstrap.sh
${_schedule} python3 /app/app/collect.py >> /dev/stdout 2>&1
${_schedule} python3 /app/app/expire.py >> /dev/stdout 2>&1
EOF
}
...
if [ ! -f /data/pg/PG_VERSION ]; then
    ...
    # The password goes through psql's own binding, not through the two shell
    # layers of su -c: interpolated inline, a $ or a quote in it produced a
    # role whose password differed from the one the app then used to connect.
    cat > /tmp/sam-create-role.sql <<'SQL'
CREATE USER :"usr" WITH PASSWORD :'pw';
SQL
    POSTGRES_USER="${POSTGRES_USER:-ssh_manager}" \
    POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-changeme}" \
    su -s /bin/sh postgres -c \
        'psql -h /tmp -v ON_ERROR_STOP=1 -v usr="$POSTGRES_USER" -v pw="$POSTGRES_PASSWORD" -f /tmp/sam-create-role.sql'
    rm -f /tmp/sam-create-role.sql
