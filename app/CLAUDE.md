...
| SAM_ADD | /usr/local/bin/sam-add \<unix_user\> \<pubkey\> | root, 750 | Crée user Unix si absent, ajoute clé idempotent (#164). `passwd -d` permet au user de définir son mot de passe via `passwd` après 1ère connexion SSH. `usermod -aG sam-users` bloque le SSH par mot de passe via le bloc sshd Match Group sam-users créé par provision-host.sh (#383) |
